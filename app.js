...
const scrollButton = document.getElementById('scroll-to-works');
...
async function loadGallery() {
  try {
    ...
    renderFilters(Array.from(tagsSet));
    renderGallery(items);
  } catch (err) {
...
function renderFilters(tags) {
  ...
  tags.forEach(tag => {
    ...
    chip.addEventListener('click', () => {
      ...
      const filtered = items.filter(item => item.tags && item.tags.includes(tag));
